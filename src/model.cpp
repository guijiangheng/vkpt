...
template <>
struct hash<vkpt::Model::Vertex> {
  size_t operator()(vkpt::Model::Vertex const &vertex) const {
    size_t seed = 0;
    vkpt::hashCombine(seed, vertex.position, vertex.color, vertex.normal,
                      vertex.uv);
    return seed;
  }
};
...
void Model::Builder::loadModel(std::string filepath) {
